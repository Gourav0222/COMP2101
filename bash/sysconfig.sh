...
function error-exit {
  echo "There is some error and need to close the program" >&2
  exit 2
}
#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
  echo -e "--help: Will Show Help\n--host: Will show host details\n--domain: Will show domain details\n--ipconfig: Will Show IP Information\n--os: Will Show OS Details\n--cpu: Will Show CPU Details\n--memory: Will Show Memory Details\n--disk: Will Show Disk Details\n--printer: Will Show Printer Details" >&2
}
