...
localAddress=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
externalAddress=$(curl -s icanhazip.com)
localHostname=$(getent hosts $localAddress | awk '{print $2}')
externalName=$(getent hosts $externalAddress | awk '{print $2}')

cat << EOF
Hostname        : $(hostname)
LAN Address     : $localAddress
LAN Hostname    : $localHostname
External IP     : $externalAddress
External Name   : $externalName
EOF

routerAddress=$(echo $localAddress | awk -F "." '{print $1 "." $2 "." $3 ".1"}')
